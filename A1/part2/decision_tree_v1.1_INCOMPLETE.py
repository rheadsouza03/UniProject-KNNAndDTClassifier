...
class DecisionTree:
    ...
    def _split_data(self, dataset: pd.DataFrame, feature_name, split_value):
        ...
        for split in split_value:
            subsets.append(dataset[dataset[feature_name] == split])
        return subsets
        #
    ...
    def _find_best_split(self, class_counts, dataset, features):
        ...
        for feature in features:
            ...
            split_values = list(set(dataset[feature]))
            print(len(split_values))
            subsets = self._split_data(dataset, feature, split_values)
            ...
            for subset in subsets:
                subset_entropy.append(self._entropy(self._calculate_class_counts(subset['class'])))
            #
            ...
            gain = self._info_gain(parent_entropy, subset_entropy, [len(subset) for subset in subsets])
        ...
        return parent_entropy, best_information_gain, best_feature, split_values #TODO change split value back
    ...
    def _build_tree(self, data, features, node: _TreeNode, current_tree_depth=0):
        ...
        print(node.class_count)
        if not ((node.class_count[0] == 0) | (node.class_count[1] == 0)):
            # Splitting the data at the best feature
            entropy, information_gain, split_feature, split_values = (
                self._find_best_split(node.class_count, data, features))
            # left_subset, right_subset = self._split_data(data, split_feature, 0)
            subsets = self._split_data(data, split_feature, split_values)

            self._build_script += f"{indentation}feature {current_tree_depth+1}[{split_feature}] (IG: {information_gain:.4f}, Entropy: {entropy:.4f})\n"
            features.remove(split_feature)

            print(len(split_values))
            child_nodes = []
            for i in range(len(subsets)):
                child_node = self._TreeNode(self._calculate_class_counts(subsets[i]['class']), parent_node=node,
                                       split_feature=split_feature, split_feature_value=split_values[i])
                self._build_script += f"{indentation}--feature {current_tree_depth + 1}[{split_feature}] == {child_node.split_feature_value}--\n"
                self._build_tree(subsets[i], features.copy(), child_node, current_tree_depth + 1)
            ...
            # self._build_script += f"{indentation}--feature {current_tree_depth+1}[{split_feature}] == {left_node.split_feature_value}--\n"
            ...
            # self._build_script += f"{indentation}--feature {current_tree_depth+1}[{split_feature}] == {right_node.split_feature_value}--\n"
